...
#Моя функция
def refreshId(timeToUpdate):
	print("Запускаю рандомные эмодзи каждую минуту")
	while (True):
		randomId = random.randint(1, 508)
		info = c.execute('SELECT * FROM "Base" WHERE id=?', (randomId, ))
		if info.fetchone() is None: 
			#Делаем когда нету id в бд
			print("Подбор нового эмодзи")
			try:
				refreshId(timeToUpdate)
			except:
				pass
		else:
			#Делаем когда есть id в бд
			set(randomId)
			sleep(int(timeToUpdate))
...
#подключение БД
conn=sqlite3.connect('vk_status.get_image_list.db')
c=conn.cursor()
